...
def run() -> tuple[bool, str]:
    luna = find_luna()
    rom = rom_path(ROM)
    # scope_con (u8) / scope_shothraw / scope_shotvraw (u16) — names
    # resolve in luna (v1.7.0).
    ok, det = assert_mem(luna, rom, STEPS, [("scope_con", "01")],
    ...
    ok, det = assert_mem(luna, rom, STEPS,
                         [("scope_shothraw", word_bytes(AIM_X_RAW)),
                          ("scope_shotvraw", word_bytes(AIM_Y_RAW))],
